...
import ru.Day4.Companion.checkStr
import ru.Day4.Companion.checkStrByAnagrams
 ...
 */
class CodeAdventDay4Test {

val puzzle ="nyot babgr babgr kqtu kqtu kzshonp ylyk psqk\n" +
    ...
    @Test
    fun testCheckStr() {
        assertTrue(Day4.checkStr("aa bb cc"))
        assertFalse(Day4.checkStr("aa bb cc aa"))
        assertFalse(Day4.checkStr("aa bb cc cc"))
        assertFalse(Day4.checkStr("aa bb cc bb"))
    }

    @Test
    fun testCheckStrAnagram() {
        assertTrue(Day4.checkStrByAnagrams("aa bb cc"))
        assertFalse(Day4.checkStrByAnagrams("abcde xyz ecdab"))
    }

    @Test
    fun testCheckAll() {
        assertEquals(325, Day4.checkAll(puzzle, ::checkStr))
        assertEquals(119, Day4.checkAll(puzzle, ::checkStrByAnagrams))
    }

    @Test
    fun testSort(){
        assertEquals("abcd",Day4.sortLetters("abcd"))
        assertEquals("abcd",Day4.sortLetters("abdc"))
        assertEquals("abcd",Day4.sortLetters("cdab"))
    }
}
...
class Day4() {
    companion object {
        ...
        fun sortLetters(str: String): String {
            return str.asIterable().sorted().joinToString("")
        }

        fun checkStr(str: String): Boolean {
            val xz = str.split(" ")
            return xz.count() == xz.distinct().count()
        }

        fun checkStrByAnagrams(str: String): Boolean {
            val words = str.split(" ").map { sortLetters(it) }

            return words.count() == words.distinct().count()
        }

        fun checkAll(str: String, checkFun: (String) -> Boolean ): Int {
            return str.split("\n").filter { checkFun(it) }.count()
        }
