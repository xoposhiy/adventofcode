...
/**
 * Created by skorokhodov on 07.12.2017.
 */
...
"""
    ...
    @Test
    fun test1() {
        assertEquals("tknk", Day7.getBottomTower(towers))
        assertEquals("cqmvs", Day7.getBottomTower(towers2.trim()))
    }

    @Test
    fun test2() {
        assertEquals("tknk", Day7.findUnbalanced(towers))
        assertEquals("bntzksk", Day7.findUnbalanced(towers2.trim()))
    }
...
class Day7 {
    companion object {
        fun findUnbalanced(t: String): String {
            var towers = t.split("\n").map { Tower.fromStr(it) }

            val weights = hashMapOf<String,Int>()
            towers.forEach{ weights[it.name] = it.weight }

            towers = towers.filter { it.items.isNotEmpty() }

            while (towers.isNotEmpty()) {
                val leafTowers = getLeafElem(towers)

                leafTowers.forEach { tower ->
                    val firstChild = tower.items.first()
                    if (tower.items.any { weights[it] != weights[firstChild] }) {
                        // нашли несбалансированную башню
                        println("error ${tower.name}")
                        tower.items.forEach { println ("$it ${weights[it]}") }
                        return tower.name
                    } else {
                        weights[tower.name] = weights[tower.name]!! + weights[firstChild]!! * tower.items.size
                    }
                    tower.items = listOf()
                }
                    towers = towers.filter { it.items.isNotEmpty() }
            }
            return "unbalanced not found"
        }

        private fun getLeafElem(towers: List<Tower>): List<Tower> {
            val roots: List<String> = towers.map{ it.name }
            return towers.filter { !it.items.any { roots.contains(it) } }
        }


        fun sumSubTree(towers: List<Tower>, root: String) : Int {
            var summa: Int = 0
            var l = listOf<String>(root)

            do {
                val res = towers.filter { l.contains(it.name) }
                summa += res.map{it.weight}.reduce { acc, i -> acc + i }
                l = res.flatMap { it.items }
            } while(l.isNotEmpty())
            return summa
        }


        fun getBottomTower(t: String): String {
            val towers = t.split("\n").map { Tower.fromStr(it) }

            val childrens = towers.flatMap { it.items }
            val roots = towers.map { it.name }.toMutableList()

            roots.removeAll(childrens)
            if (roots.size > 1) {
                println(roots)
                throw Exception("Ошибка что-то пошло не так... ")
            }
            return roots.first()
        }
    }
...
class Tower(val name: String, val weight: Int, _items: String) {
    var items: List<String> = if (_items.isNotBlank()) _items.split(",").map { it.trim() } else listOf()


    companion object {
        fun fromStr(str: String): Tower {
            val firstSplit = str.trim().split("->")
            val items = if (firstSplit.size == 2) firstSplit.last().trim() else ""
            val nameAndWeight = firstSplit.first().trim().split(" ")
            val name = nameAndWeight.first().trim()
            val weight = nameAndWeight.last().trim().removePrefix("(").removeSuffix(")")

            return Tower(name, weight.toInt(), items)
        }
    }

    override fun toString(): String {
        return "\nTower(name='$name', weight=$weight, items=[${items.joinToString(", ")}]"
    }
}